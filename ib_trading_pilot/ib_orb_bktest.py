...
def backtest(top_gap_by_date, data, app):
    ...
    for date in top_gap_by_date:
        ...
        for ticker in top_gap_by_date[date]:
            ticker_event.clear()
            histData(reqID,usTechStk(ticker),date+" 22:05:00 US/Eastern",'1 D', '5 mins')
            ticker_event.wait()
            ...
            if app.skip == False:
                ...
                for i in range(1,len(app.data[reqID][1:])):
                    if app.data[reqID].iloc[i-1]["Volume"] > 2*(data[ticker].loc[date,"AvVol"])/78 \
                       and app.data[reqID].iloc[i]["High"] > hi_price \
                       and open_price == '':
                        open_price = 0.8*app.data[reqID].iloc[i+1]["Open"] + 0.2*app.data[reqID].iloc[i+1]["High"] #factoring in slippage
                        direction = 'long'
                    elif app.data[reqID].iloc[i-1]["Volume"] > 2*(data[ticker].loc[date,"AvVol"])/78 \
                       and app.data[reqID].iloc[i]["Low"] < lo_price \
                       and open_price == '':
                        open_price = 0.8*app.data[reqID].iloc[i+1]["Open"] + 0.2*app.data[reqID].iloc[i+1]["Low"] #factoring in slippage
                        direction = 'short'
                        
                    if open_price != '' and direction == 'long':
                        if app.data[reqID].iloc[i]["High"] > hi_price*1.05:
                            ticker_return = ((hi_price*1.05)/open_price)-1
                            date_stats[date][ticker] = ticker_return
                            break
                        elif app.data[reqID].iloc[i]["Low"] < lo_price:
                            ticker_return = (lo_price/open_price) - 1
                            date_stats[date][ticker] = ticker_return
                            break
                        else:
                            ticker_return = (app.data[reqID].iloc[i]["Close"]/open_price) - 1
                            date_stats[date][ticker] = ticker_return
                            
                    if open_price != '' and direction == 'short':
                        if app.data[reqID].iloc[i]["Low"] < lo_price*0.95:
                            ticker_return = 1 - ((lo_price*0.95)/open_price)
                            date_stats[date][ticker] = ticker_return
                            break
                        elif app.data[reqID].iloc[i]["High"] > hi_price:
                            ticker_return = 1 - (hi_price/open_price)
                            date_stats[date][ticker] = ticker_return
                            break
                        else:
                            ticker_return = 1 - (app.data[reqID].iloc[i]["Close"]/open_price)
                            date_stats[date][ticker] = ticker_return
