...
# wrapper function for reqHistoricalData. this function gives the candle historical data
    def historicalData(self, reqId, bar):
        if reqId not in self.hist_data:
            self.hist_data[reqId] = pd.DataFrame([{"Date":bar.date,"Open":bar.open,"High":bar.high,"Low":bar.low,"Close":bar.close,"Volume":bar.volume}])
        else:
            self.hist_data[reqId] = pd.concat((self.hist_data[reqId],pd.DataFrame([{"Date":bar.date,"Open":bar.open,"High":bar.high,"Low":bar.low,"Close":bar.close,"Volume":bar.volume}])))
...
# wrapper function for reqExecutions.   this function gives the executed orders
    def execDetails(self, reqId, contract, execution):
        ...
        self.execution_df = pd.concat((self.execution_df, pd.DataFrame([dictionary])),ignore_index=True)
    ...
    while not kill_event.is_set():
        for ticker in tickers:
            ...
            if app.inExec(ticker) == 0 and app.tickerAllOpenOrders(ticker) == 0 and not hist_event.is_set():
                ...
                if 1.1*app.av_volume[ticker] < last_volume:
